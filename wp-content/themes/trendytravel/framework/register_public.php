...
function dt_theme_enqueue_scripts() {
	...
	echo "\n \t\t,framework_base_url:'".$template_uri."/'";
	...
	if(dt_theme_option('general', 'disable-retina-support') != true)
	...
	wp_enqueue_script('jq.uitotop', $template_uri.'/js/public/jquery.ui.totop.min.js',array(),false,true);
	...
	if(dt_theme_option('general', 'disable-style-picker') != "on") {
		...
		wp_enqueue_script('jq.colorbox', $template_uri.'/js/public/jquery.colorbox-min.js', array(), false, true);
	...
	wp_enqueue_script('jq.fitvids', $template_uri.'/js/public/jquery.fitvids.js', array(), false, true);
	wp_enqueue_script('jq.bxslider', $template_uri.'/js/public/jquery.bxslider.js', array(), false, true);

	wp_enqueue_script('jq.parallax', $template_uri.'/js/public/jquery.parallax-1.1.3.js', array(), false, true);
	wp_enqueue_script('jq.animatenum', $template_uri.'/js/public/jquery.animateNumber.min.js', array(), false, true);
	wp_enqueue_script('jq.tooltipster', $template_uri.'/js/public/jquery.tooltipster.min.js', array(), false, true);

	if(is_singular('dt_hotels') || is_singular('dt_places')):
	    wp_enqueue_script('jq.googleapi', 'http://maps.google.com/maps/api/js?sensor=false', array(), false, true);
		wp_enqueue_script('jq.googlemap', $template_uri.'/js/public/jquery.gmap.min.js', array(), false, true);
	endif;

	if( is_page_template('tpl-onepage.php') )
		wp_enqueue_script('jq.onepage.nav', $template_uri.'/js/public/jquery.nav.js', array(), false, true);

	if(dt_theme_option('general', 'disable-hotel-booking') != "on")
		wp_enqueue_script('hb-booking-script', $template_uri.'/js/public/booking.script.js', array(), false, true);
...
function dt_theme_slider_section($post_id) {
	...
	if(array_key_exists('show_slider',$tpl_default_settings) && array_key_exists('slider_type',$tpl_default_settings) ):
		...
		if( array_key_exists("show_search_section",$tpl_default_settings) ):
			if( !array_key_exists("search_section_on_slider", $tpl_default_settings) ):
				echo "<div class='fullwidth-section grey-bg' style='padding:44px 0px 30px;'>";
					echo "<div class='container'>";
						require_once $template_uri."/includes/hotels-searchform.php";
					echo '</div>';
				echo "</div>";
...
add_action ( 'comment_post', 'dt_add_hotel_comment_meta_values', 1 );
function dt_add_hotel_comment_meta_values( $comment_id ) {
    if ( isset( $_POST['title'] ) ) {
        $title = wp_filter_nohtml_kses( $_POST['title'] );
        add_comment_meta( $comment_id, 'title', $title, false );
    }
    if ( isset( $_POST['profession'] ) ) {
        $profession = wp_filter_nohtml_kses( $_POST['profession'] );
        add_comment_meta( $comment_id, 'profession', $profession, false );
    }
}
 ...
 *		To customize the post/page comments view.
 **/
function dt_theme_hotel_comments($comment, $args, $depth) {
	$GLOBALS['comment'] = $comment;
	switch ( $comment->comment_type ) :
	   case 'pingback' :
  	   case 'trackback' :
...
class dt_theme_breadcrumb {
	...
	function simple_breadcrumb_case($der_post) {
			...
			if (is_attachment()) {
				...
				if ($postType == 'post') {
					...
					if ($postType == 'dt_galleries') {
						...
						if ($postType == 'dt_hotels') {
							global $post;
							$terms = get_the_term_list($post->ID, 'hotel_entries', '', '$$$', '');
							$terms = array_filter(explode('$$$', $terms));
							if (!empty($terms)) :
								echo $terms[0].$markup;
							endif;
		...
		if (is_post_type_archive('product')) {
			return __("Packages", 'iamd_text_domain');
		}

		if (is_post_type_archive('dt_hotels')) {
			return __("Hotels", 'iamd_text_domain');
		}
