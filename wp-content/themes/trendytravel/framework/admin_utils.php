...
function dt_theme_default_option() {
	$general = array (
			...
			"top-bar-left-content" => '<p><i class="fa fa-phone"></i>Any Questions? Call Us: 1-223-355-2214</p>',
			"copyright-text" => '&copy; 2016 - Trendy Travel - by <a href="http://themeforest.net/user/designthemes" title="">Design Themes</a>'
	);
...
// # --- **** dt_theme_hotels_categorylist() *** --- ###
function dt_theme_hotel_categorylist($id, $selected = '', $class = "mytheme_select") {
	$name = explode ( ",", $id );
	if (count ( $name ) > 1) {
		$name = "[{$name[0]}][{$name[1]}]";
	} else {
	...
	$output = "<select name='{$name}' class='{$class}'>";
	$output .= "<option value=''>" . __ ( 'Select Category', 'iamd_text_domain' ) . "</option>";
	$cats = get_categories ( 'taxonomy=hotel_entries&hide_empty=0' );
	foreach ( $cats as $cat ) :
		@$id = esc_attr ( $cat->term_id );
		@$title = esc_html ( $cat->name );
		@$output .= "<option value='{$id}' " . selected ( $selected, $id, false ) . ">{$title}</option>";
...
// # --- **** dt_theme_hotels_list() *** --- ###
function dt_theme_hotelslist($id, $selected = '', $class = "mytheme_select") {
	$name = explode ( ",", $id );
	if (count ( $name ) > 1) {
		$name = "[{$name[0]}][{$name[1]}]";
	} else {
	...
	$name = ($class == "multidropdown") ? "mytheme{$name}[]" : "mytheme{$name}";
	$output = "<select name='{$name}' class='{$class}'>";
	$output .= "<option value=''>" . __ ( '--- Select Hotels ---', 'iamd_text_domain' ) . "</option>";
	$myhotels = get_posts ( 'posts_per_page=-1&post_type=dt_hotels' );
...
// # --- **** dt_theme_destinationslist() *** --- ###
function dt_theme_destinationslist($id, $selected = '', $class = "mytheme_select") {
	$name = explode ( ",", $id );
	if (count ( $name ) > 1) {
		$name = "[{$name[0]}][{$name[1]}]";
	} else {
	...
	$name = ($class == "multidropdown") ? "mytheme{$name}[]" : "mytheme{$name}";
	$output = "<select name='{$name}' class='{$class}'>";
	$output .= "<option value=''>" . __ ( '--- Select Destinations ---', 'iamd_text_domain' ) . "</option>";
	$myplaces = get_posts ( 'posts_per_page=-1&post_type=dt_places' );
	
	foreach ( $myplaces as $place ) : setup_postdata( $place );
		@$id = esc_attr ( $place->ID );
		@$title = esc_html ( $place->post_title );
		@$output .= "<option value='{$id}' " . selected ( $selected, $id, false ) . ">{$title}</option>";
	endforeach;
